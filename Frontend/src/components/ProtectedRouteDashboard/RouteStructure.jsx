...
function RouteStructure({ routes = [] }) {
  // Group routes by base
  const groupedRoutes = routes.reduce((acc, curr) => {
    const base = curr.route?.base || "unknown";
    const sub = curr.route?.sub;
    const child = curr.route?.child;

    if (!acc[base]) acc[base] = [];

    if (sub) acc[base].push(`/${sub}`);
    if (child) acc[base].push(`/${sub}/${child}`);
    ...
    <div className="bg-black border border-zinc-900 rounded-lg px-6 py-5 shadow-inner mb-5">
      ...
      <div className="space-y-4 text-sm font-mono text-zinc-300">
        {Object.keys(groupedRoutes).length === 0 ? (
          <p className="text-zinc-500 text-sm text-center">No routes added yet</p>
        ) : (
          Object.entries(groupedRoutes).map(([base, subs]) => (
            <div key={base}>
              {/* BASE */}
              ...
              {/* CHILDREN */}
              <div className="ml-6 mt-2 space-y-1 border-l border-zinc-800 pl-4">
                {[...new Set(subs)].map((subRoute, idx) => (
                  <div key={idx} className="flex items-center justify-between">
                    <span className="text-zinc-400">{subRoute}</span>
